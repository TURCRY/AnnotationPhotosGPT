...
if page == "Génération standard":
    ...
    if st.button("▶️ Exécuter"):
        ...
        if endpoint == "/annoter_rag":
            ...
            payload["top_k"] = int(st.session_state.get("k_ragvec", 5))
            ...
            if "show_distances" in st.session_state:
                payload["show_distances"] = bool(st.session_state["show_distances"])
            if "max_total_chars" in st.session_state and int(st.session_state["max_total_chars"]) > 0:
                payload["max_total_chars"] = int(st.session_state["max_total_chars"])
...
# --------------------------------------------------
    ...
    with tabs[1]:
        ...
        if up and st.button("⬆️ Envoyer au serveur"):
            ...
            data = r.json()
            if data.get("ok"):
                st.success(f"{data.get('filename') or up.name} envoyé")
                st.json({
                ...
                st.error(data.get("error", "Upload impossible."))
                st.json(data)
...
if uploaded_files and st.button("⬆️ Envoyer tous les fichiers"):
    for f in uploaded_files:
        ...
        try:
            ...
            data = r.json()
            if data.get("ok"):
                st.success(f"{data.get('filename') or f.name} envoyé")
                st.caption(f"{data.get('path','')} ({data.get('size', 0)} octets)")
            else:
                st.error(f"{f.name}: {data.get('error', 'Upload impossible.')}")
        except Exception as e:
