...
def main():
    ...
    infos_path = Path(args.infos_projet_json).resolve()
    infos = load_json(infos_path)
    ...
    photos_batch_csv = Path(
        pc.get("fichier_photos_batch")
        or infos.get("fichier_photos_batch")
        or str(photos_csv.with_name(photos_csv.stem + "_batch.csv"))
    )

    # si absent dans pcfixe, inscrire et sauvegarder le même JSON passé en argument
