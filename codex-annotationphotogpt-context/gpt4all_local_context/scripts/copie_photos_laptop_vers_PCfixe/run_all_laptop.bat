...
REM    (Ne pas faire confiance au .csv des photos pour ces 2 champs)
REM ======================================================
...
REM ======================================================
REM 1) LECTURE DU .CSV DES PHOTOS (source de vérité)
REM ======================================================
...
REM ======================================================
REM 2bis) Dossier AUDIO (pour trouver contexte_general_photos.json) - existant
REM ======================================================
for /f "usebackq delims=" %%i in (`
...
REM Dossier transcription PC fixe (pour déposer prompt + infos_projet normalisé)
...
REM Controle: on verifie bien le fichier qui sera copie
echo DEBUG APRES post_sync, PHOTOS_CSV=[%PHOTOS_CSV%]
dir /-c "%PHOTOS_CSV%"

REM ======================================================
REM 5) COPIE DES JPG NATIFS + REDUITS (conserve dates/attributs; EXIF conserve)
REM ======================================================
...
REM ======================================================
REM 5bis) COPIE DES WAV (source + compatible) vers dossier audio canonique PC fixe
REM ======================================================
...
REM ======================================================
REM 6) COPIE DU .CSV DES PHOTOS (+ XLS/XLSX SI PRESENT) + ANNOTATIONS (noms reels)
REM ======================================================
echo [4/6] Copie CSV + xls/xlsx + annotations vers le PC fixe...

REM copie du CSV sous son nom réel
...
REM copie du .xls ou .xlsx si présent
...
REM ======================================================
REM 7) COPIE prompt_gpt.json + prompt_gpt_batch_only.json + DEPOT infos_projet.json NORMALISE (PC fixe autonome)
REM ======================================================
...
REM Snapshot config LLM pour le batch
...
REM Contexte PHOTOS + contexte compte rendu (source : dossier transcription si possible, sinon audio)
...
REM ======================================================
REM 7bis) COPIE TRANSCRIPTIONS ET ANNEXES (csv/srt/vtt/progress/proper_names/participants/sujets/etc.)
REM ======================================================
...
REM ======================================================
REM 7ter) GARDE-FOU: si fichier_transcription contient (photo).csv, tenter le meme nom sans (photo)
REM ======================================================
python -c "import json; from pathlib import Path; p=Path(r'%DST_INFOS%'); d=json.load(p.open(encoding='utf-8')); v=d.get('fichier_transcription',''); \
if isinstance(v,str) and v.strip(): \
    pv=Path(v); \
    if pv.name.endswith('(photo).csv'): \
        alt = pv.with_name(pv.name.replace('(photo).csv','.csv')); \
        if alt.exists(): d['fichier_transcription']=str(alt); \
p.write_text(json.dumps(d, indent=2, ensure_ascii=False), encoding='utf-8')"

REM ======================================================
REM Normalisation: remplace les chemins laptop par des basenames (pour batch PC fixe)
REM ======================================================
