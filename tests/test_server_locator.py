...
from app import server_locator
...
class ServerLocatorTests(unittest.TestCase):
    ...
    def test_resolve_prefers_first_endpoint_that_answers_ping(self):
        ...
        with patch("app.server_locator.requests.get", side_effect=fake_get):
    ...
    def test_cache_reuses_working_endpoint(self):
        with patch("app.server_locator.requests.get", return_value=SimpleNamespace(ok=True)) as mock_get:
    ...
    def test_request_retries_on_network_failure(self):
        ...
        with patch("app.server_locator.requests.get", side_effect=fake_get):
            with patch("app.server_locator.requests.request", side_effect=fake_request):
